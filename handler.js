...
export async function handler(chatUpdate) {
  ...
  /* Creditos a Otosaka (https://wa.me/522221745133) */
    ...
    case 'remove':
      if (chat.welcome && !chat?.isBanned) {
        ...
        for (const user of participants) {
          let pp = 'https://raw.githubusercontent.com/Santx07/Propio/master/src/avatar_contact.png';
          try {
  ...
  for (const nk of callUpdate) {
    if (nk.isGroup == false) {
      if (nk.status == 'offer') {
        ...
        const vcard = `BEGIN:VCARD\nVERSION:3.0\nN:;Santx07 👑;;;\nFN:Santx07 👑\nORG:Santx07 👑\nTITLE:\nitem1.TEL;waid=522221745133:+51 992 621 601\nitem1.X-ABLabel:Santx07 👑\nX-WA-BIZ-DESCRIPTION:[❗] ᴄᴏɴᴛᴀᴄᴛᴀ ᴀ ᴇsᴛᴇ ɴᴜᴍ ᴘᴀʀᴀ ᴄᴏsᴀs ɪᴍᴘᴏʀᴛᴀɴᴛᴇs.\nX-WA-BIZ-NAME:Santx07 👑\nEND:VCARD`;
        await mconn.conn.sendMessage(nk.from, { contacts: { displayName: 'Santx07 👑', contacts: [{ vcard }] } }, { quoted: callmsg });
